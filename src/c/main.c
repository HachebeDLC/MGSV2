...
//
// The background is generated, not photographed: tools/make_background.py
// draws the AGAM601's LCD filling the whole screen, with no watch case around
// it - the same choice the Watchy MGSV face makes. The geometry block below
// is emitted by that script; re-run it if the artwork changes.
//
//   LCD window   x 13..96,  y 13..118   (octagon, 6px chamfer)
//   rail         x 110..142             (six labelled boxes, then a free box
//                                        and the battery bar)
//   lower band   y 133..166
// ---------------------------------------------------------------------------

// generated by tools/make_background.py - do not hand-edit
#define LCD_X          13
#define LCD_Y          13
#define LCD_W          83
#define LCD_H          105
...
// The sweep runs just inside the LCD border.
#define RING_X         LCD_X
#define RING_Y         LCD_Y
#define RING_W         LCD_W
#define RING_H         LCD_H

// The emblem is printed at the LCD centre, so the two clear strips inside the
// window are above it and below it. Marks are 1/14 of the LCD, so readouts
// keep clear of that band.
#define LY_SEC_X       (LCD_X + 8)
#define LY_SEC_Y       (LCD_Y + 8)
#define LY_SEC_W       (LCD_W - 20)
#define LY_SEC_H       26

// A TextLayer reserves the font's empty ascender, so the glyphs land about
// 8px below the box origin - hence starting level with the emblem's foot.
#define LY_TIME_X      (LCD_X + 14)
#define LY_TIME_Y      (EMBLEM_CY + EMBLEM_R - 2)
#define LY_TIME_W      (LCD_W - 20)
#define LY_TIME_H      32

#define LY_AMPM_X      (LCD_X + 2)
#define LY_AMPM_Y      (EMBLEM_CY + EMBLEM_R + 8)
#define LY_AMPM_W      12
#define LY_AMPM_H      18
...
#define LY_BATT_W      (RAIL_W - 2)
#define LY_BATT_H      BAR_H
...
static void hands_update_proc(Layer *layer, GContext *ctx) {
  ...
  int32_t c   = sx(LCD_CHAMFER);   // exact chamfer from the generated art
  if (c < 2) c = 2;
